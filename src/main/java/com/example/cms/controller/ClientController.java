...
//            byte[] b = decoder.decodeBuffer(faceBase64);
//            for (int i = 0; i<b.length; ++i) {
//                if (b[i] < 0) {
//                    b[i] += 256;
//                }
//            }
//            OutputStream out = new FileOutputStream("tempTest.jpg");
//            out.write(b);
//            out.flush();
//            out.close();
...
//                BASE64Decoder decoder = new BASE64Decoder();
//                byte[] faceBytes = decoder.decodeBuffer(faceBase64);    //解码
//                for (int i = 0; i < faceBytes.length; ++i) {  //调整异常数据
//                    if (faceBytes[i] < 0) {
//                        faceBytes[i] += 256;
//                    }
//                }
...
//                BufferedImage faceBufferedImage = ImageIO.read(file); //文件转bufferedImage
                ...
                if (!StringUtils.isEmpty(faceFeature)) {
                    personService.updateFaceImg(phoneNum, faceBytes);
                    personService.updateFaceFeature(phoneNum, faceFeature);
                    responseJson.put("resultCode", -1);
                } else {
                    responseJson.put("resultCode", 1);
                }
